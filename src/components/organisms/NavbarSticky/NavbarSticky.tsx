...
export interface NavbarStickyProps {
    ...
    onCartClick?: () => void;
    onSearchFocus?: (kind: "store" | "product", slug: string) => void;
}
...
export function NavbarSticky({
    ...
    const handleSearchSelect = (suggestion: SearchSuggestion) => {
        ...
        if (sectionHref && targetSlug && kind) {
            ...
            const focusDelay = sectionHref === "#vitrina" ? 700 : 500;
            window.setTimeout(() => {
                if (onSearchFocus) {
                    onSearchFocus(kind as "store" | "product", targetSlug);
                }
            }, focusDelay);
        }
    };
                            ...
                            <div className={navbarStickySearchVariants({ active: searchActive })}>
                                <SearchBar
                                    ...
                                    onSubmit={(val) => {
                                        ...
                                        else window.open(`https://jovenpro.com/?s=${encodeURIComponent(val)}&post_type=product`, "_blank");
                                    }}
